    ...
    def check_mmd(self):
        ...
        if "mmd:last_metadata_update" in mmd:
            if isinstance(mmd["mmd:last_metadata_update"], dict):
                ...
                if myvalue is None:
                    raise ValueError(
                        "Date could not be parsed: %s", mmd["mmd:last_metadata_update"]
                    )
                ...
                for mykey, myitem in mmd["mmd:temporal_extent"].items():
                    ...
                    if (myitem is None) or (myitem == "--"):
                        ...
                        try:
                            ...
                            mmd["mmd:temporal_extent"][mykey] = mydate.strftime(
                                "%Y-%m-%dT%H:%M:%SZ"
                            )
                        except Exception as e:
                            logger.error("Date format could not be parsed: %s", e)
    ...
    def tosolr(self):
                ...
                if len(latvals) > 0 and len(lonvals) > 0:
                    ...
                    if max(latvals) == min(latvals):
                        if max(lonvals) == min(lonvals):
                            point = shpgeo.Point(
                                float(e["mmd:rectangle"]["mmd:east"]),
        ...
        if "mmd:personnel" in self.mydoc["mmd:mmd"]:
            ...
            for role in personnel_role_LUT:
                ...
                mydict[
                    "personnel_{}_address_province_or_state".format(personnel_role_LUT[role])
                ] = []
            ...
            for personnel in personnel_elements:
                ...
                for entry in personnel:
                    ...
                    if entry_type == "role":
                        mydict["personnel_{}_role".format(personnel_role_LUT[role])].append(
                            personnel[entry]
                        )
                        ...
                        if entry_type == "contact_address":
                            for el in personnel[entry]:
                                ...
                                if el_type == "address":
                                    mydict[
                                        "personnel_{}_{}".format(personnel_role_LUT[role], el_type)
                                    ].append(personnel[entry][el])
                                else:
                                    mydict[
                                        "personnel_{}_address_{}".format(
                                            personnel_role_LUT[role], el_type
                                        )
                                    ].append(personnel[entry][el])
                            ...
                            mydict[
                                "personnel_{}_{}".format(personnel_role_LUT[role], entry_type)
                            ].append(name)
        ...
        if "mmd:related_information" in mmd:
            ...
            for related_information in related_information_elements:
                ...
                if value in related_information_LUT.keys():
                    ...
                    if (
                        "related_url_{}".format(related_information_LUT[value])
                        not in mydict.keys()
                    ):
                    ...
                    mydict["related_url_{}".format(related_information_LUT[value])].append(
                        related_information["mmd:resource"]
                    )
                    ts = "mmd:description"
                    if ts in related_information and related_information[ts] is not None:
                        mydict[
                            "related_url_{}_desc".format(related_information_LUT[value])
                        ].append(related_information[ts])
                    else:
                        mydict[
                            "related_url_{}_desc".format(related_information_LUT[value])
                ...
                if isinstance(thumbClass, dict) and thumb_impl == "fastapi":
                    ...
                    if error is None and status_code == 200:
                        ...
                        if thumbnail_url is not None:
                            logger.debug(
                                "Adding thumbnail_url field with value: %s", thumbnail_url
                            )
                        ...
                        logger.error(
                            "Could not generate thumbnail, reason: %s, status_code %s",
                            error,
                            status_code,
                        )
                    ...
                    logger.debug(
                        "Creating WMS thumbnail using legacy method using url: %s", getCapUrl
                    )
